...
NumericVector sparse_sim(IntegerVector path, double nugget,
                         NumericVector w_, NumericMatrix Bi_,
                         NumericMatrix KMi_, double maxvar,
  ...
  for(int i = 0; i < path.length(); i++){
    ...
    // double v = arma::as_scalar(sum(k % (Bi * k))) + vplus(path(i));
